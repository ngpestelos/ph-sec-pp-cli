...
var whichIndex = []whichEntry{
	{Command: "company show", Description: "Show registration status for a known SEC number from the local cache, with optional live refresh.", Group: "Local state that compounds", WhyItMatters: "Use when you already know the SEC number and want status without re-hitting the API every time."},
	{Command: "which", Description: "State clearly what this free-only CLI does and does not do (no name search, no GIS/AFS).", Group: "Agent-native plumbing", WhyItMatters: "Call this before assuming name search or filings extracts exist."},
	{Command: "search", Description: "Full-text search previously synced company status rows without network calls.", Group: "Local state that compounds", WhyItMatters: "Use after sync when you need to filter known companies offline."},
	{Command: "sql", Description: "Run SQL against the local company status store.", Group: "Local state that compounds", WhyItMatters: "Use for ad-hoc filters on cached free-product data."},
	{Command: "doctor", Description: "Check PH_SEC_TOKEN presence and remind free-product scope and quota.", Group: "Agent-native plumbing", WhyItMatters: "Run first when setup fails; free is not unauthenticated."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  ph-sec-pp-cli which "stale tickets"
  ph-sec-pp-cli which "bottleneck"
  ph-sec-pp-cli which --limit 1 "send message"
  ph-sec-pp-cli which                                # list the full capability index`,
